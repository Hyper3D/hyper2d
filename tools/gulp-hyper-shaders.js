...
module.exports = function (src) {
			...
			if (match) {
				switch (match[1]) {
					case 'parameter':
                        ...
                        if (!match[2].match(/^c_.*/)) {
                            this.emit('error', new PluginError('gulp-hyper-shaders',
                                'Parameter name should start with "c_": ' + match[2]));
                        }
						break;
					case 'require':
					...
					case 'attribute':
                        ...
                        if (!match[2].match(/^a_.*/)) {
                            this.emit('error', new PluginError('gulp-hyper-shaders',
                                'Attribute name should start with "a_": ' + match[2]));
                        }
						break;
                    case 'uniform':
                        shader.uniforms.push(match[2]);
                        if (!match[2].match(/^u_.*/)) {
                            this.emit('error', new PluginError('gulp-hyper-shaders',
                                'Uniform name should start with "u_": ' + match[2]));
                        }
                        break;
					default:
	...
	function processInput(file, enc, cb) {
		...
		processShader.call(this, text, file);
